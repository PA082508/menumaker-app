...
## [MED] Compliance-map: чего не хранит License-трекер

Сверка с бумажной лицензией Pearl (18.07). В базе есть и совпадает:

| поле | бумага | база |
|---|---|---|
| номер лицензии | 000000300629 | ✅ совпадает |
| выдана | 10/06/2014 | ✅ `license_issued = 2014-10-06` |
| ёмкость / под 2½ | 158 / 36 | ✅ `license_capacity` / `license_capacity_under2` |

**Не хранится нигде** — кандидаты в compliance-map:

- **статус «Continuous»** (тип/срок действия лицензии) — колонки нет;
- **Administrator: Cynthia Patsko + Tatiana Kogan** — двое, а в базе поля под
  администратора лицензии нет вовсе (в Parent Handbook они есть текстом);
- **бланк JFS 01256 (rev. 12/2016)** — редакция формы не фиксируется, а именно
  она устаревает при смене бланка ODJFS.

**⚠️ Отдельная находка, не про хранение:** `fso_license_expires` у Pearl =
**2026-03-01**, то есть FSO-лицензия (Cuyahoga County Board of Health,
`MJAE-9N5L63`) числится **истёкшей четыре месяца назад**. Либо продлена и база
не обновлена, либо это реальная просрочка. Проверить по бумаге — это тот класс
поля, где «не обновили в базе» и «просрочено» выглядят одинаково.
