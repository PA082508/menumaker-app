...
## STAFF / HR — три заказа 07.08, спеки бумагой

Все три живут в [`specs/2026-08-07-staff-hr-vault.md`](specs/2026-08-07-staff-hr-vault.md):

| | Заказ | Очередь |
|---|---|---|
| **1** | **Мини-фикс «два вычислителя часов»** — Total = сумма строчных Hours, единственный вычислитель = строка | **окно белой Cancel** (ближайшее) |
| **2** | **Payroll Privacy Gate (org-level)** — оклад орг-персонала только под собственным PIN, замок СЕРВЕРНЫЙ | спека+показ **после показа инбокс-папок** |
| **3** | **Holiday pay + порог стажа** — праздничные не оплачиваются первые 3 месяца; часы ≠ деньги | по слову, вместе со слоем начислений |

⚠️ По (2) замер уже есть и он неприятный: `hourly_rate` читается из PostgREST **любой**
вошедшей сессией — прячет его только UI. `pin_set_at` / `pin_hash` на том же запросе дают 403,
то есть нужный механизм в проекте уже применён к PIN и просто не применён к деньгам.

## ЭТАП 2 App учителя — STEP OUT / STEP IN

Особое поручение директора: часы не рвутся, предусловие выхода — подмена в комнате, след санкции
обязателен. Записано разделом в [`specs/2026-08-06-teacher-app-v1.md`](specs/2026-08-06-teacher-app-v1.md).

## TRUSTED QR — персональный QR доверенного взрослого (заказ 07.08, спека бумагой)

**Очередь: ПОСЛЕ спеки App учителя (`51d012a`), стройка по слову владельца. НЕ блокер
понедельника.** Канон и честное состояние — строкой в [`DECISIONS.md`](DECISIONS.md); временный
мост (одноразовый код директора) — в [`guides/gatepulse-director-guide.md`](guides/gatepulse-director-guide.md).

Рамка, как заказана:

- **генерация** — с issue-страницы: кнопка «QR card» у доверенного без смартфона;
- **носитель** — персональный токен **класса device-токенов** (не новый стор);
- **выдача** — печатная PDF-карта (крупное фото + QR, стиль Doc Hub) **или** «сохранить
  картинкой на телефон»;
- **скан** — камерой учительского планшета, **та же камера**, что фото-захват;
- **учителю** — фото + имя → сверка лицом → Release;
- **отзыв** — Revoke, мгновенный, уносит фото;
- **типы поэтапно, постоянный первым** (постоянный · на период · одноразовый).

Замеры, которые обязаны войти в показ: хранение связи токен ↔ лицо · срок жизни токена ·
честный отказ **словами** при отозванном или чужом скане.

## Очередь после починки ключа — три строки, добавленные 31.07

Замеры и обоснования: [`plans/2026-07-31c-frp-carrier-and-claim-surfaces.md`](plans/2026-07-31c-frp-carrier-and-claim-surfaces.md).

| | Работа | Оценка | Когда |
|---|---|---|---|
| **1** | **ОДИН СЧЁТЧИК НА ОБА ВЫХОДА** — Site Claim Report переводится на `compute_monthly_claim`, поклассовая разбивка уезжает в RPC, клиентский счётчик удаляется | **≈1 день** | 🔴 **ДО подачи июля** — пока счётчика два, бумага и заявка могут разойтись, и узнается это после подачи |
| **2** | **Перенос сборки листа в приложение** — генератор повторяет `displayChildName`, компаратор возрастных полос и правило возмещения отдельным кодом на JS | 1 день + 0.25 на служебный лист | **сразу после починки ключа** |
| **3** | **Ёмкость: одно хранилище** — `compute_monthly_claim` читает `center_licenses.child_care`, строка есть только у Pearl; Ridge и Highland получают `null` в claim-facing поле C9 | 0.25 fallback + 0.25 сверка | после заведения лицензий Ridge/Highland (нужны и к 15.10) |

⚠️ **Порядок в (3) обратен интуиции:** истинное хранилище — `center_licenses` (ёмкость есть
свойство лицензии, с номером, сроком и файлом), но сегодня оно пусто у двух центров из трёх.
Значит сперва бумага, потом снятие витрины `centers.license_capacity`; до того — падение назад
на витрину, а не `null` в поле заявки.

## ⏰ ВЕХА 15 ОКТЯБРЯ 2026 — ежегодное обновление и ресертификация заявки CRRS

**Отдельная веха, не строка очереди** (решение владельца 31.07). Ложится на окно боевого
старта 1 октября — даты рядом, но разные: **1.10** система впервые подаёт месяц сама,
**15.10** — условие того, что подавать вообще будет кому.

- **Источник:** бюллетень Ohio DEW «CACFP Policy Updates», 30.07.2026, + политика
  «Annual CRRS Application Updates Policy». **Статус: подтверждается у EPS.**
- ⚠️ **Прежний известный срок — 15 ноября** (18 лет практики владельца). Расхождение
  зафиксировано явно и не затирается.
- **Цена пропуска:** продолжение участия в CACFP, включая подачу месячных заявок на
  возмещение → остановка денег по трём центрам.
- **Новых форм НЕТ:** родительский и кадровый кит не затронут. Затронут спонсорский уровень
  — комплект к загрузке в CRRS.
- **Готовность комплекта на 31.07: три текущие лицензии с файлами из девяти ожидаемых**,
  у Highland Heights ни одной строки, актов инспекций ноль.

Замер, чек-лист недостающего и спека реестра сроков —
[`specs/2026-07-31-regulatory-deadlines-registry.md`](specs/2026-07-31-regulatory-deadlines-registry.md);
разбор четырёх политик — [`compliance-map.md`](compliance-map.md).

## СРОК ИЮЛЬ 2027 — ежегодный пересмотр DCY 01234 впервые заработает

DCY 01234 — форма **новая от июля 2026**, поэтому строки пересмотра сегодня пусты и
заполниться не могут: первые пересмотры наступают **в июле 2027**. У трёх задач теперь есть
дата, а не «когда-нибудь»:

1. **Аффорданс ИНИЦИАЛОВ в приложении** — норма требует инициалы + дату от родителя И
   администратора (OAC 5180:2-12-15); в приложении аффорданса нет вовсе, `CountersignField` —
   пад полной подписи, то есть другой юридический акт. Копировать прецедент недельного
   meal count (`director_initials` + штамп). См. [renewal-contour-spec §7.5](specs/renewal-contour-spec.md).
2. **Сборщик листа из цепочки записей** — [§7.6.4](specs/renewal-contour-spec.md): для человека
   один лист с тремя строками, для базы цепь неизменяемых записей, связанных `relation='renewal'`.
   Оценить: порядок строк · прерванная цепочка · продление без инициалов администратора.
3. **Хвост кита «первая свободная строка всегда даёт row 1»** — не укусит до июля 2027,
   но чинится вместе с renewal-контуром. ⚠ Отдельно: **одна запись** уже израсходовала все три
   строки (проставлены датой первичной подписи, до появления гейта) — сборщик должен отличать
   их по признаку «дата строки = дата подписи», см. [§7.6.5](specs/renewal-contour-spec.md).

## ГРАНИЦА ИЗДАНИЙ В ДАННЫХ (действует сейчас)

Подписанное **до июля 2026** — старая редакция бланка; **после** — новая. Учитывать в
провенансе и при любом сравнении форм разных лет: одинаковый ключ формы не означает одинаковый
набор полей.

## ВОПРОС (не дефект): должен ли печатный лист нести предупреждения (2026-07-28)

Аллергии читаются **только на экране** отметки питания — жёлтая плашка `⚠` в
[`MealCountPage.tsx:759`](../src/pages/meal-count/MealCountPage.tsx). На печатных листах их нет
**не потому что сломалось, а потому что так спроектировано**: ни один печатный шаблон это поле
не берёт.

Вопрос владельцу, оценка — не стройка: **должен ли печатный лист нести предупреждения**, и если
да — **какие именно и в какой форме**, чтобы не перенести на бумагу тот же шум. Сегодня 6 из 16
плашек сообщают, что предупреждать не о чем, а одна (Roman Guarnera) активно успокаивает о
ребёнке с четырьмя препаратами. Перенести это на печать значит размножить проблему, а не решить.

Разумный порядок: сначала разделение поля на четыре (флаг «аллергий нет» · аллергии · диета ·
замена молока), потом решение о печати — тогда на лист попадёт то, что действительно
предупреждение.

## MAINTAINER — отчётный линт «catch, кончающийся console.*» (2026-07-28)

Второе правило из пары про тихие отказы. **Сначала МЕРИТЬ, потом запрещать**: линт выдаёт
**список нарушителей** — `catch`-блоки, которые заканчиваются `console.*` и не обновляют
состояние экрана, — и **сборку не валит**. Когда список известен и осмыслен, решаем, становится
ли правило жёстким.

Первое правило пары (голый `await supabase…insert/update/upsert/delete/rpc` без связывания
результата) идёт жёстким сразу — оно поймало бы все пять тихих отказов недели.

## MAINTAINER — ежемесячный прогон «колонка без формы / поле без потребителя» (2026-07-28)

Из канона [«Пункт назначения — запись ребёнка»](platform-standards.md). Два списка, дельта
к прошлому месяцу:

1. **колонки-сироты** — поля карточки, которых не собирает ни одна действующая форма;
2. **поля-сироты формы** — ключи, которые форма собирает, а платформа не читает.

Растущий список означает, что порядок «от данных» вернулся. Это **постоянная проверка**,
а не разовый аудит.
...
и по списку имён политик это неотличимо от защищённой (канон
[«PERMISSIVE-политика не есть защита»](platform-standards.md)). Красным: таблица, у которой
**ноль** restrictive-политик.
...
## Кит DCY 01234 — строки пересмотра не накапливаются по годам (2026-07-28)

Гейт редакции строк пересмотра стоит и работает (`f3b4c03`): на первичной подаче три строки
заперты и под watermark, на renewal (`?t=`) открывается **первая свободная**. Недодел: **без
prefill'а уже заполненных строк по токену «первая свободная» всегда даёт row 1**, поэтому
накопление по годам (2026 → row 1, 2027 → row 2, 2028 → row 3) сегодня не работает — каждый
пересмотр пишется в первую строку.

Норма требует именно накопления: OAC 5180:2-12-15 — ежегодный пересмотр с инициалами и датой
обеих сторон, три строки на бланке ровно под три года.

Чинится **в ките** (`pa082508.github.io`), **вместе с renewal-контуром**, не сейчас.
Связано: [renewal-contour-spec §7.2, §7.5](specs/renewal-contour-spec.md).
...
⚑ **Важная поправка к серьёзности (замер 28.07): все три таблицы ПУСТЫ — 0 / 0 / 0 строк.**
Значит **ни одна детская запись через это видна не была**: дефект в правиле, не утечка данных.
Формулировку «директор Ridge видит детей Pearl» читать как «правило это разрешает», а не как
случившееся. И то же окно, что у типизированного ребра: пока пусто — обе работы (org-паритет
и разграничение по центру) стоят ноль бэкфилла, а окно закроется тихо, как только формы начнут
подавать.

**Разделено на две работы (Николай, 28.07):**
- **(а) `milk_substitutions` без org-изоляции = дефект, не выбор.** Паритет с двумя сёстрами —
  `docs/prepare/20260728e_milk_substitutions_org_isolation.sql`, ждёт go. Ничего не ждёт само.
- **(б) разграничение ПО ЦЕНТРУ для всех трёх — проектная работа:** нужен `center_id` и связь
  с ростером. Место — **вместе с заходом 1**, который как раз строит связь ребёнка.
  Отдельно не начинать.

**Калибровка серьёзности (слова владельца, для спеки):** сегодня организация одна, вход только
у своих директоров и админов — это **внутренняя приватность, а не внешняя утечка**. Но детские
диетические и медицинские сведения через границу центра — то, к чему придерётся инспектор,
а таблица без org-фильтра — **мина под мульти-тенант**.
...
## Политика, опирающаяся на всегда пустую колонку, — это видимость защиты

**Найдено 29.07 замером, при сужении `dispatch_routes`.** На таблице стояла политика записи
`dr_manage → (org_id is not null and core.is_org_member(org_id))`. Выглядит как арендатор.
На деле **все 8 строк несли `org_id = NULL`** — предикат не срабатывал ни разу за всё время
жизни таблицы, и записи под ним не происходило никогда. Больше того: у роли `authenticated`
на этой таблице **грант только SELECT**, так что права под политикой не было и подавно.
Политика была декоративной **дважды**.

**Почему это отдельный класс, а не частный случай.** В любом обзоре политик такая строка
читается как «запись защищена арендатором» — и она правда защищена, только защищать нечего:
условие ложно всегда. Ровно так же выглядела бы политика, честно охраняющая живые данные.
**Отличить одно от другого можно только замером колонки, на которую политика опирается.**

**Признак для обзора (в чек №2 maintainer'а, к «нет restrictive» и «permissive не упоминает
арендатора»):** для каждой политики, чей предикат опирается на колонку `X`, — доля строк
с `X IS NULL`. **100 % = политика ничего не удерживает.** Между 0 и 100 — частичная слепота,
и это тоже находка.

**Смежное, тем же признаком:** политика, выданная роли **без гранта** на нужное действие
(`dr_manage` на `ALL` при гранте `SELECT`), — та же видимость. Проверять пару
«политика + грант», а не политику отдельно.

Разбор — **по слову**, отдельным заходом; сегодня 29.07 закрыт только `dispatch_routes`
(бэкфилл `org_id` + `NOT NULL` + сужение, `20260729c`), прав никому не добавлено.


## CI в репозитории форм — вторая половина защиты полки образцов

**Триггер: при следующем касании `pa082508.github.io`.** Не «когда-нибудь» — именно тогда, потому
что руки уже будут в том репозитории.

**Почему это нужно, хотя послеполёт уже смотрит.** 29.07 два сборочных гарда
(`sampleScopeGuard`, `kitVersionFloor`) вынесены из сборки приложения: они читают соседний
checkout, которого на Vercel нет, и валили **каждую** выкладку. Проверка переехала в послеполёт,
и это **правильный артефакт** — там смотрят на то, что **развёрнуто на Pages**, а не на локальную
копию.

**Но честный противовес:** послеполёт **обнаруживает, а не предотвращает**. Перевернут
`SAMPLE_SCOPE` и выложат — машинерия жива до ближайшего прогона. Предотвращение возможно только
там, где происходит изменение: **CI в самом репозитории форм**.

**Что делать (≈1 ч):** workflow в `pa082508.github.io`, падающий, если
`var SAMPLE_SCOPE` ≠ `'none'` или если хоть одно включение `form-kit.js?v=` ниже пола. Тогда
получится пара: **CI предотвращает у источника, послеполёт обнаруживает на выкладке.**

## `sched_source` / `sched_updated_by` / `sched_updated_at` — самодельный провенанс, написанный до журнала

**Отмечено 29.07 при описи писателей `roster`, в работу НЕ берётся.**

Три колонки на `roster` держат то же, что теперь держит `child_field_events`: **кто, когда и на
каком основании** изменил расписание. Они написаны **раньше** журнала и с тех пор живут рядом.

**Теперь провенанса два**, и это тот же класс «две реализации одного правила», который 29.07
поймался дважды за день: `documentDateOf` в `dcyPort` (второе имя, другое поведение) и порядок
чтения в `enrollmentValidationRules` (третья копия). **Такие пары расходятся молча — всегда,
вопрос только когда.**

**Что решать, когда дойдут руки:** свести к журналу (расписание пишется через
`record_child_field_change`, три колонки выводятся из событий) — или назвать их **осознанным
исключением** с причиной, как мы делаем со списками исключений. Третьего («пусть лежат») быть не
должно: незамеченная пара опаснее обеих реализаций порознь.


## 164 расхождения имени в отметках — это ПОРЯДОК ИМЕНИ, и «чинить» их нельзя

**Классификация (владелец, 29.07), чтобы не починили в неверную сторону.**

Замер: 164 строки `meal_week_records`, где `child_name` в отметке ≠ `child_name` в ростере.
Вид расхождения: **`Myla Marra` против `Marra Myla`** — это **не смена имени и не описка**, это
**порядок имени**, вчерашний коллапс «Имя Фамилия» ↔ «Фамилия Имя».

**Почему приводить к одному виду — неверно.** По канону **хранимый ключ не пересобирается**, а
порядок имени **применяется на рендере**. Значит правильное направление не «выровнять 164 строки»,
а **перестать хранить в отметке склеенное имя вовсе**: у 96 % строк уже есть `roster_id`, и имя
там — вторая копия, которая по построению будет расходиться.

**Направление (по слову):** отметка держит `roster_id`, имя выводится при показе и печати; 92
строки без `roster_id` (см. заход «перенос охраны») сперва получают ключ. **Не выравнивать.**

- **Staff-набор в `packet_sets`** (по слову владельца). Сегодня состав `?set=staff` живёт в реестре (`packets.staff.slots`) и правится только коммитом в двух зеркалах; строки `staff` в `packet_sets` нет, поэтому Set Builder её не показывает. Перевод даст владельцу править состав пакета сотрудника своей рукой, как детские наборы.
- **Настоящая форма «Add Staff»** (по слову владельца). Кнопка Add Staff — экран пакета и QR; создания сотрудника из веба НЕТ ни одного (`insert` в `staff` в приложении отсутствует), поэтому новую строку сегодня заводит только SQL. Нужна форма с полями и галкой «организационный уровень» при заведении.
- **Сведение права забирать к ОДНОМУ носителю** (по слову владельца). Сегодня «право забирать» живёт в двух местах: `child_guardian.can_pickup` (пришло импортом, ключ → `child.id`) и `safepass_trusted_persons` (право у двери, ключ → `roster.id`). Разные ключи и два источника правды об одном и том же. До сведения карточка родителя ПОКАЗЫВАЕТ ОБА и называет расхождение вслух; править `can_pickup` из карточки нельзя.
- **Конструктор писем v2** (строка по слову владельца, 06.08). Владелец вставляет текст письма в приложении → витринная страница с подписным блоком публикуется сама, самообслуживанием, в духе Set Builder. Предпосылка — каркас «Letter with signature» (см. `docs/specs/2026-08-06-letter-with-signature.md`): пока каркас не построен, конструктору нечем управлять.

## ПРАВКА ОТМЕТОК ПИТАНИЯ ЗАДНИМ ЧИСЛОМ — копилка 07.08, бумага

Формула целиком — [`specs/2026-08-07-meal-marks-amendment.md`](specs/2026-08-07-meal-marks-amendment.md):
PIN директора · причина словами · штамп «amended after service» в самой записи · видимость правок
в недельном подтверждении директора · счётчик правок как метрика дисциплины. Point-of-service
остаётся правилом. **Предусловие смысла:** отметка должна быть ИМЕННОЙ, то есть после PIN-входа
в оболочку `/teacher` (App учителя v1). Стройка по слову, не раньше.

## ЧИСТКА ДУБЛЕЙ РАСПИСАНИЯ — малым окном по слову (замер 07.08)

Блокер Presence board и любого серверного подсчёта часов: у Raegan Smith 10 активных строк
`staff_schedules` на 5 дней (legacy + интервал на каждый день). Победитель пары — строка с
интервалом; legacy-дубль **деактивируется, не удаляется**. Подробности и вопрос показа —
[`specs/2026-08-07-staff-hr-vault.md`](specs/2026-08-07-staff-hr-vault.md), пункт 4.

## PIN-МЕХАНИЗМ — разделение возможностей и реестр точек (канон 07.08, бумага)

[`specs/2026-08-07-pin-capability-split.md`](specs/2026-08-07-pin-capability-split.md): PIN
подтверждает личность внутри роли, а не повышает её; проверка «роль+точка» до verify PIN; реестр
точек тремя колонками; директорский PIN — директорский класс в своём центре; список возможностей —
конфиг владельца. **Замер в спеке:** живые точки перечислены (передача · чек-ин/аут · транспорт ·
одноразовый код · заведение PIN), явной проверки «роль+точка» в них сегодня нет. Питает Payroll
Gate (пункт «в») и правку отметок питания.

## APP УЧИТЕЛЯ v1 — спека-показ готова (07.08), стройка по слову

[`specs/2026-08-07-teacher-app-v1-show.md`](specs/2026-08-07-teacher-app-v1-show.md): оболочка
`/teacher` с тремя вкладками под одним входом, PIN на входе (порядок «роль+точка» до PIN), явная
смена человека + таймаут, «Моё время» только часы через единственный вычислитель. **Четыре вопроса
владельцу** до стройки: величина таймаута · что видит вошедший без комнаты (13 из 73) · второй
планшет под понедельник (рабочий сегодня один) · подтверждение, что meal-флоу не меняется.

## ПОНЕДЕЛЬНИК — список людских актов (не стройка)

- **Перезавести PIN'ы Saniya Clark и Maureen Minadeo через карточку** (Соня): PIN у них стоит, а
  `pin_set_at` / `pin_set_by` пусты — заведены до появления штампа, и «кто завёл» неотвечаем.
  Forward-only: прошлое не переписываем, след появится сам при перезаводе.
- **Второй планшет Red** — по желанию владельца: для старта хватает одного, резерв регистрируется
  через Settings → Devices за минуту.
- Правки текстов владельца → **выпуск издания писем одним заходом** (ветки
  `fix/texts-monday-edition-20260807` в обоих репо).
- Учётка Татьяны.
