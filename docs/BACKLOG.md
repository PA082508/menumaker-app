...
## Имя клеймовой колонки динамическое с обеих сторон — проверять нечем

**Замер 29.07 (read-only).** Приложение собирает имя из частей — `` `${day}_${SLOT_COL[slot]}` ``
(`MealCountDirectorPage`), база принимает его **из полезной нагрузки клиента** — `%1$I` в
`sync_meal_marks`, и читает тем же способом в `delivery_reconciliation` (`served`).

**Полного имени колонки нет ни в дереве, ни в теле функции.** Все шесть наших гардов текстовые —
для этого места они слепы **все сразу**.

**Риск не в безопасности** (`%I` квотирует, несуществующая колонка даёт явную ошибку), **а в
клеймовой цифре**: другое **валидное** имя (`mon_l` вместо `mon_b`) положит отметку в соседний
приём пищи, и не заметит никто.

**Направление лечения (по слову):** белый список из 30 колонок **внутри** `sync_meal_marks` —
проверка там, где имя настоящее, а не там, где оно собирается. Полчаса, форвардная миграция.
