...
1. 🔴 **Часть отметок пишется МИМО журнала — старой кэшированной сборкой PWA.** Поправка
   31.07: первоначально названный адрес (`MealCountDirectorPage.tsx`) **неверен** — файл мёртв,
   обе двери рендерят `MealCountPage`. В текущем коде путь один — очередь → `sync_meal_marks`,
   которая пишет клетку и журнал вместе. Но до коммита `f25d55d` (10.07) `toggle` писал клетку
   **напрямую**, и устройство, не получившее новую сборку, продолжает так писать. За недели
   20.07 и 27.07 у Ridge в журнале **ноль строк** при живой сетке — то есть по этому центру
   требование «time of service» **сегодня не исполняется и не измеряется**.
2. **Своевременность замерена:** неделя 20–24.07, Highland + Pearl, 1 795 отметок —
   **38 % во время подачи**, 10 % наперёд, 34 % позже в тот же день, **17 % на другой день**
   (макс. опоздание 31 день). **За вычетом сверочных правок — 42 / 11 / 38 / 8 %**: «на другой
   день» падает вдвое, и людям предъявляется именно очищенная доля. Разбор: [план 31.07d](plans/2026-07-31d-pos-timeliness-measure.md).
...
## Прецедент проверки: подача раньше объявленного времени → ОТКАЗ В ВОЗМЕЩЕНИИ (владелец, 31.07)

**Собственный случай центра.** Во время проверки один класс приступил к еде **раньше на 5–10
минут**. Нарушение зафиксировано, **в возмещении за этот приём отказано**.
*(Дата и центр — уточняются у владельца; в документы для персонала не идут.)*

**Почему это стоит рядом с нормой.** §226.15(e)(4) и §226.17(b)(9) требуют счёта **во время
подачи**, но не называют цены. Прецедент называет: **цена — отказ в деньгах за конкретный
приём**, у нас, при живой проверке.

### ⭐ Следствие: время в `meal_schedule` — ГРАНИЦА НАРУШЕНИЯ, а не линейка для метрики

| | Требование | Связь |
|---|---|---|
| 1 | Расписание в Настройках **обязано совпадать** с заявленным штату в **site application CRRS** | иначе версий две, и проверяющий пользуется своей. **Веха 15.10** — ежегодное обновление CRRS: одно и то же расписание в двух местах, вносится одним заходом |
| 2 | Вписать 11:30, когда класс ест в 11:20, = **узаконить нарушение** | приводить в соответствие **в обе стороны**: либо время подачи, либо запись — и сразу в обоих местах |

Развёрнуто: [спека недельного листа §18](specs/2026-07-29-weekly-meal-count-sheet.md).
