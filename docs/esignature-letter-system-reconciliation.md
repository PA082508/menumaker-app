...
| "Every record can be **printed on demand in the state form layout**" | approved forms with a replica now **print from a frozen snapshot** (official pages 1:1, screen-only provenance banner), else a labelled live render | ✅ **Implemented 2026-07-25** (Step 3; DCY 01234 verified end-to-end by record — row + bucket object + edge-fn log). Other types as replicas are added. |
