...
begin
  ...
  -- ВТОРОЙ СЛУЧАЙ 06.08: миграция 20260806q добавила staff.is_org_level и НЕ
  -- выдала на неё грант — карточка снова падала целиком (`permission denied for
  -- table staff`), потому что права здесь ПОКОЛОНОЧНЫЕ. Проба S-1 в её нынешнем
  -- виде это ловит: она собирает список колонок из information_schema, значит
  -- новая колонка попадает в запрос сама. Ловит — если её ЗАПУСТИТЬ.
  -- ПРАВИЛО ОКНА: миграция, добавляющая колонку в staff, обязана (а) выдать
  -- грант той же миграцией и (б) закрыться прогоном этого блока, а не «зелёным
  -- verify» приложения — vitest базы не видит.
