...
| **СОРТИРОВКА** | **ВСЕГДА по `last_name`.** Именно это даёт соседство сиблингов, а НЕ порядок показа: трое Coleman встанут рядом при любом отображении |
...
**Известное расхождение, ждёт слова:** ось «сортировка всегда по `last_name`» расходится
с §2a ниже, где CACFP-контексты сортируются по возрасту (`byAgeOldestFirst`, разведён по
страницам meal count). Приведение §2a к новой оси **изменит порядок строк на печатных
листах meal count** — claim-facing, поэтому кодом не трогается до отдельного слова.
