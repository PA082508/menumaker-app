...
## ⭐ СТРОИШЬ ФОРМУ — БЕРИ ТО, ЧТО НА ЭКРАНЕ (Николай, 2026-07-31)

**Это правило стоит ВЫШЕ всех прочих правил о формах. Форма собирается ОТ ЭКРАНА:
теми же именами, тем же порядком строк, тем же составом итогов, что видит и подписывает
человек. Экран — РЕАЛИЗАЦИЯ правил, а не их конкурент.**

**Проверка приёмки — одна и физическая:**

```
положить рядом ЭКРАН и ФОРМУ
расхождение в порядке имён · порядке строк · составе итогов = ФОРМА НЕ ПРИНЯТА
```

**Почему это правило первое, а не одно из.** Сверка владельца 31.07: экран **уже** соблюдает
всё, что записано канонами — имена «Фамилия Имя», сортировка по возрасту, итог «всего / к
клейму» (27/29). Недельный лист, собранный **мимо** экрана, нарушил четыре правила сразу,
и каждое из них экран исполнял в тот же момент на соседней вкладке. **Открывать каноны не
пришлось бы вовсе, если бы форма строилась от экрана** — проход по канонам (правило ниже)
остаётся, но он вторая линия, а не первая.

⚠️ **Что это НЕ значит.** Не «читать пиксели». Значит: та же функция показа имени, тот же
компаратор сортировки, тот же счётчик итогов — **общий код, а не повторённая логика**.
Повторённая логика расходится по определению; расхождение подписанной бумаги с экраном
директора и есть дефект, ради которого правило записано.

⚠️ **Порядок при конфликте.** Если экран расходится с каноном — чинится **экран**, и форма
идёт за ним. Форма никогда не «исправляет» экран в одиночку: тогда подписывают одно, а
считают другое.
...
## ПЕРЕД ГЕНЕРАЦИЕЙ ЛЮБОЙ ФОРМЫ — ПРОХОД ПО КАНОНАМ (Николай, 2026-07-31)

**Перед генерацией ЛЮБОЙ формы — обязательный проход по канонам:
`platform-standards` · спека этой формы · канон формата имени (§1) и оси сортировки (§2).
Отчёт о форме несёт строку «ПРИМЕНЁННЫЕ ПРАВИЛА» рядом со строкой `template source`.
Форма без этой строки НЕ ПРИНИМАЕТСЯ.**

```
ПРИМЕНЁННЫЕ ПРАВИЛА: §1 показ «Фамилия Имя» из частей · §2a возрастные полосы,
внутри полосы last_name · spec §8 п.7 кружок · spec §8 п.9 одна функция счёта ·
«рабочие данные из окон» · своих текстов на бланке нет
template source: <ключ реестра + издание>
```

Строка перечисляет правила **по именам разделов**, а не словом «проверено»: проверяющий
обязан суметь открыть каждое названное место. Правило, которое к форме **не** применимо,
в строку не пишется — строка есть перечень применённого, а не список прочитанного.

**Прецедент, из-за которого правило записано (31.07).** Недельный лист meal count был
сгенерирован **без единого открытия правил**, и все четыре нарушенных правила лежали в
репозитории — часть **моими же коммитами**: показ имени (§1, «Фамилия Имя» из частей),
ось сортировки (§2a, возрастные полосы — коммит `54419d3`, мой), кружок исключённого
приёма (spec 29.07 §8 п.7), запрет своих текстов на официальном бланке. Форма выглядела
готовой и была неприёмной: **сходство с бланком не есть соответствие бланку**.

⚠️ Смежное правило: `Ready-made forms first` отвечает на вопрос «откуда шаблон», это —
на вопрос «по каким правилам он заполнен». Обе строки печатаются вместе, и ни одна не
заменяет другую.

## РАБОЧИЕ ДАННЫЕ БЕРУТСЯ ИЗ ОКОН (Николай, 2026-07-31)

**Экран — то, что видят и подписывают люди; он источник для форм, отчётов и счёта.
Всё, что добыто мимо экрана (SQL, `information_schema`, сырые строки, прямые выгрузки), —
ДИАГНОСТИКА: помечается как служебное, не попадает в официальные формы и в счёт без слова
владельца.**

```
❌ ЗАПРЕЩЕНО: официальный бланк, собранный из выборки, которой ни одно окно не показывает
❌ ЗАПРЕЩЕНО: число в отчёте владельцу, у которого нет окна-предъявителя
✅ ДИАГНОСТИКА НЕ ОТМЕНЯЕТСЯ — именно она нашла имя в ключе строки недели
```

Правило **не запрещает** SQL и замер — оно отделяет их от рабочего продукта:

| | Диагностика | Рабочий продукт |
|---|---|---|
| Откуда | SQL, схема, сырые строки | окно приложения |
| Куда идёт | план, спека, записка «замерено» | форма, отчёт, клейм |
| Как помечено | явно «служебное / не официальный документ» | ничем — это и есть документ |
| Кто пускает дальше | **слово владельца** | — |

**Следствие для служебных пометок.** Диагностический знак (спорная строка, подсветка,
пояснение метода) на официальном бланке жить не может: тексты формы вместе с её названием
имеют юридическую силу. Такие пометки идут на **ОТДЕЛЬНЫЙ служебный лист**, явно
помеченный как не-официальный, либо вне рамки бланка.

**Следствие для «источник — строки недели» (spec meal count §A5).** Противоречия нет:
после перевода экрана на союз (`ростер ∪ строки недели` по `roster_id`) экран **и есть**
строки недели. До этого перевода два множества расходятся — и это ровно причина, по
которой лист выпускается **после** починки ключа, а не раньше.
...
## ИМЯ В КЛЮЧЕ СТРОКИ — ЭТО ГЕНЕРАТОР СИРОТ (Николай, 2026-07-31)

**Идентичность записи не строится на человекочитаемом поле.** Ключ — это `roster_id`, `child_id`,
`id`; имя, написание, метка — **подпись**, а не ключ.

**Случай, породивший правило.** `sync_meal_marks` объявлял идентичность недельной строки как
`on conflict (classroom_id, child_name, monday_date)`. `roster_id` в строке **был**, но
идентичность стояла не на нём. Следствие цепочкой:

```
импорт кладёт «Имя Фамилия» → ростер несёт «Фамилия Имя»
→ экран ищет records[roster.child_name] и НЕ НАХОДИТ → неделя выглядит пустой
→ повар отмечает её заново → on conflict не срабатывает (имя другое)
→ ВТОРАЯ строка. Первая становится невидимой и неправимой навсегда
→ клейм суммирует обе
```

Измерено: **115 клеток июля** на строках-сиротах, $244.30. Ни одного тапа мимо интерфейса — все
нажатия настоящие, в журнале точки обслуживания.

**Три свойства, которые делают это дороже обычной ошибки:**

1. **Ошибка невидима на экране** — не потому, что её прячут, а потому, что экран индексирует тем
   же испорченным ключом. Человек не может её ни увидеть, ни исправить.
2. **Она самовоспроизводится:** пустая на вид неделя провоцирует повторную отметку, а повторная
   отметка создаёт следующую сироту.
3. **Экран при этом ПРАВДИВ.** Владелец сказал «на экране 100% правда» — и был прав: экран
   показывал ровно то, что повар отметил последним. Расхождение жило **между** экраном и клеймом,
   и ни одна из сторон о нём не сообщала.

**Признак для поиска остального (уточнён 31.07 по итогам прогона всей базы).** У каждого
`on conflict` / уникального индекса / join'а спросить не «текстовая ли это колонка», а —
**кто и зачем меняет значение**:

| | Примеры | Значение меняется когда |
|---|---|---|
| **Здоровый ключ** | `slug`, `key`, `code`, `form_key`, `dedup_key`, `token` | МЫ намеренно меняем идентичность |
| **Больной ключ** | имя ребёнка, имя комнаты, e-mail, название праздника | ЧЕЛОВЕК поправил написание, не собираясь ничего расщеплять |

Прежняя формулировка признака звучала как «есть ли в ключе поле, которое человек может написать
двумя способами». Она **заменена этой**: в том виде признак давал ложные срабатывания — текстовых
ключей в базе десятки, и почти все здоровы. `slug` тоже можно написать двумя способами, но его
никто не правит из соображений вежливости к ребёнку.

**Что нашёл прогон** (полностью — `docs/plans/2026-07-31b-identity-key-sweep.md`):

- ключей на имени в `meal_week_records` оказалось **два**, а не один: второе ограничение держит в
  идентичности ещё и **текстовое имя комнаты**, и оно уже двоится в живой базе — в восьми классах
  внутри одного `classroom_id` записаны два написания («Blue» / «Blue Room», «Pre-K» / «PreK»).
  Не выстрелило только потому, что `upsert` целится в другой индекс. **Заряд снимать вместе с первым;**
- **ключ офлайн-очереди на устройстве собран из того же имени** (`cellKey`, `mealMarkQueue.ts`).
  Починка ключа в базе его не лечит: очередь живёт на устройстве и чинится своим движением;
- опекун опознаётся по имени в **143 случаях из 414** (`match_key` с префиксом `n:`);
- праздник обновляется и удаляется **по своему названию**;
- `staff_email_unique` — человекочитаемое поле в ключе **и** без org-скоупа: адрес уникален по всей
  базе, один человек не может работать в двух организациях.

**Отдельный вывод, которого признак не даёт.** Уникальный индекс по колонке, где есть NULL, **не
удерживает NULL-строки вовсе** — в Postgres NULL не равен NULL. В `meal_week_records` 92 строки живут
без `roster_id` (88 из них — «Staff Room», у персонала ростера нет). Правильный ключ обязан быть
**частичным**, а строкам без ребёнка нужен свой — иначе починка детских строк тихо снимет
ограничение с чужих.

**Порядок починки обязателен: сначала схлопнуть накопленных сирот, потом ставить правильный
уникальный индекс.** Индекс на верном ключе **не создастся** поверх данных, которые он объявляет
невозможными — та же ловушка «схема впереди данных» с другой стороны.

**И схлопывание — не техническая операция.** Замер 31.07: из 24 проигравших строк **23 несут подпись
директора**. Директор подписал обе стороны расщепления, потому что на экране видел одну строку.
Значит слияние трогает подписанное, а подписанное не переписывают: проигравшая строка уезжает в
архив **дословно** (вместе с подписью, статусом и временем), в живой сетке остаётся одна строка с
объединёнными отметками, а сам факт слияния — **новая запись**, а не правка задним числом.

**Схлопывание не применяется, пока директор правит руками, и список не зашивается в файл.** 31.07
директор тронул 174 строки недели за два часа и подписал неделю 13.07 — пять из 24 групп были бы
слиты у него под руками. Список сирот меняется день ко дню (21 класснеделя вчера → 24 группы
сегодня), поэтому миграция **вычисляет его сама в момент применения** и **падает сама**, если хоть
одна целевая строка правилась в последние 30 минут. Дисциплина не должна опираться на память.

## ⭐ ПРИВЫЧКА НЕ ЕСТЬ КАЛЕНДАРЬ (Николай, 2026-07-31)

**Регуляторный срок хранится В СИСТЕМЕ — со ссылкой на источник и датой ЕГО публикации — и
ПЕРЕПРОВЕРЯЕТСЯ ЕЖЕГОДНО. Он никогда не воспроизводится по памяти сотрудника, каким бы
длинным ни был его опыт.**

```
❌ ЗАПРЕЩЕНО: «мы всегда сдавали до 15 ноября» как основание для планирования
✅ ТРЕБУЕТСЯ: срок · дата · источник с датой публикации · когда перепроверен · кем
```

**Живой пример, ради которого правило записано.** Годовое обновление документов CRRS все годы
делалось **до 15 ноября** (18 лет практики владельца). Бюллетень DEW от **30.07.2026** называет
**15 октября** — срок сдвинулся на месяц вперёд. Обнаружилось это **случайно**, при плановой
работе о другом; по памяти о сдвиге узнали бы **после срока**.

**Цена ошибки здесь не бумажная.** Невыполнение годовых требований в срок влияет на продолжение
участия в CACFP, включая подачу месячных заявок на возмещение, — то есть **остановка денег по
трём центрам**.

**Следствия:**

1. Прежнее значение срока **не затирается** — расхождение фиксируется явно, с обеими датами и
   обоими источниками (память владельца — тоже источник, с датой и способом).
2. Срок, пришедший из бюллетеня, **живёт со статусом**: `подтверждается у EPS` — пока не
   подтверждён, он планируется как действующий, а не как слух.
3. Реестр регуляторных сроков — [спека 31.07](specs/2026-07-31-regulatory-deadlines-registry.md).
   Агент дозора работает **над реестром**, а не вместо него: он приносит изменение источника,
   решение о сроке принимает человек.

## ⭐ ПРЕЖДЕ ЧЕМ ПРЕДЪЯВЛЯТЬ ЧИСЛОМ — ПРОВЕРИТЬ МОДЕЛЬ ИЗМЕРЕНИЯ (Николай, 2026-07-31)

**Число, которым предъявляют людям, обязано пройти проверку на СООТВЕТСТВИЕ МОДЕЛИ ТОМУ, КАК
ПРОЦЕСС УСТРОЕН В ЖИЗНИ. Пока эта проверка не сделана, число не показывают ни в документе, ни в
разговоре — оно измеряет нашу модель, а не работу людей.**

```
❌ ЗАПРЕЩЕНО: «8% отмечают вовремя» до вопроса «а как этот приём вообще устроен?»
✅ ТРЕБУЕТСЯ: спросить того, кто делает работу, ЧТО ИМЕННО происходит — до того, как считать
```

**Живой пример, ради которого правило записано (31.07).** Замер своевременности отметок дал по
завтраку **8 % «во время подачи»** против 51 % у ланча. Цифра выглядела как небрежность
персонала и уже готовилась в записку **под роспись**. Владелец назвал устройство процесса:
**завтрак накопительный — дети собираются постепенно и кормятся по приходу**. К растянутому
кормлению было приложено фиксированное окно 07:00–08:00. **Худшая цифра дня оказалась ошибкой
нашей модели, а не поведением людей.**

Тем же движением выяснилось, что и остальные числа замера **провизорны**: они считались против
ОБЩИХ окон, а расписание кормления **по классам** ещё не заполнено — класс, который реально ест
в 12:00 при окне 11:30, выглядит опоздавшим, не будучи им.

**Три следствия, обязательные:**

1. **Модель называет тот, кто делает работу.** Устройство процесса — не наша гипотеза; его
   формулирует владелец/директор/повар, и до этого метрика не публикуется.
2. **Процесс-поток и процесс-событие меряются РАЗНО.** К накопительному приёму окно неприменимо
   в принципе; правильный ответ по нему — **«не измеряется»**, а не ноль.
   Ложный ноль хуже отсутствия числа: он выглядит как факт о людях.
3. **Число, посчитанное против незаполненной настройки, помечается ПРОВИЗОРНЫМ** прямо в
   документе, где оно живёт, — и не переносится в документы для людей до пересчёта.

⚠️ **Отдельно про документы под роспись.** Цена ошибки здесь не «поправим в следующей версии»:
человек подписывает утверждение о себе. Поэтому для таких документов проверка модели —
**предусловие выпуска**, наравне со сверкой нормы по источнику.

## СТРАНИЦА ДЛИННЕЕ ДВУХ ЭКРАНОВ НЕСЁТ КНОПКУ НАВЕРХ (Николай, 2026-08-05)

**Правило.** Любая страница, которая в обычной работе становится длиннее **двух экранов**,
обязана нести кнопку возврата к началу; **кнопка появляется, как только прокрутка ушла за
первый экран** — то есть видна уже на втором. Общий компонент — `src/components/common/ScrollToTop.tsx`;
второй такой кнопки в репозитории быть не должно.

**Почему это стандарт, а не украшение.** Список бумажных заявлений Wickliffe — сотня строк.
Докрутив до низа, человек возвращается к полю поиска колесом через весь экран, и делает это
десятки раз за вечер. Такая работа не видна ни в одном отчёте и не попадает ни в одну жалобу:
её просто терпят.

**Порог показа — один экран, и считается он от высоты окна** (`scrollY > innerHeight`), а не от
числа пикселей: на планшете повара и на мониторе офиса «экран» — разные величины. Поправка
владельца 05.08: было два экрана, стало один — кнопка нужна там, где верх уже ушёл из виду.
На короткой странице кнопки нет вовсе: прокручивать нечего, а место она занимает.

**Прокрутка плавная, кроме случая `prefers-reduced-motion: reduce`** — там мгновенный прыжок.
Это не пожелание к оформлению: для части людей плавная прокрутка через сотню строк вызывает
тошноту.

**Кнопка не печатается** (`class="no-print"`) и не участвует в подписанных документах.

Применено 05.08: `/iea-confirm`, `/children`, `/parents`. Каждая новая длинная страница
добавляет компонент сама — отдельного разрешения на это не требуется.
