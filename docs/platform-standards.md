...
## У каждого гейта есть когорта «до» (Николай, 2026-07-28)
...
Случай, на котором правило родилось: одна запись с преждевременно проставленными строками
...
по `menumaker.roster`»*. Правила поведения (что делать) даты не требуют — требует её
