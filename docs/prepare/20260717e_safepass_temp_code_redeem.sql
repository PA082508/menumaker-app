...
-- READ-BACK: 1·t·t·f
-- VERIFY:   select (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
--                    where n.nspname='menumaker' and p.proname='safepass_redeem_temp_code') = 1
--                  as rpc_exists,
--                  has_function_privilege('anon','menumaker.safepass_redeem_temp_code(text,text)','execute')
--                  as anon_can_execute,
--                  not exists (select 1 from pg_policies where schemaname='menumaker'
--                               and tablename='safepass_temp_codes' and 'anon' = any(roles))
--                  as no_anon_policy;
--
...
-- ── SELF-CHECK (урок 18.07: do-блок не доехал при вставке — проверка живёт в файле) ──
-- Не меняет данные. Если вставка оборвалась — этот блок просто не вернёт 4 строки.
select 'function'      as what, (count(*))::text as got, '1' as expect
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'menumaker' and p.proname = 'safepass_redeem_temp_code'
union all
select 'grant anon',
       has_function_privilege('anon',
         'menumaker.safepass_redeem_temp_code(text,text,text)', 'execute')::text, 't'
union all
select 'grant authenticated',
       has_function_privilege('authenticated',
         'menumaker.safepass_redeem_temp_code(text,text,text)', 'execute')::text, 't'
union all
select 'anon select on temp_codes revoked',
       has_table_privilege('anon', 'menumaker.safepass_temp_codes', 'select')::text, 'f';
