...
-- ---------------------------------------------------------------------------
-- §1b ЗАРЯДКА ПЛАНШЕТА — что сделать с плейнтекстом, пока консоль открыта
--
--     URL-шаблон (подставить токен из §1 вместо <ТОКЕН>):
--
--       https://<домен-приложения>/safepass/teacher?device_token=<ТОКЕН>
--
--     Три шага, на планшете, в один присест:
--       1. Открыть этот URL на iPad комнаты Red (тот самый планшет, не свой).
--       2. Убедиться, что кнопки Accept / Release ОЖИЛИ (перестали быть серыми)
--          и красной полосы «device not registered» нет.
--       3. Убедиться, что `?device_token=` ИСЧЕЗ из адресной строки — страница
--          вычищает его сама. Если он остался — токен не сохранился, повторить
--          шаг 1; не оставлять URL с токеном в истории планшета.
--
--     Кто и когда: Николай — вс, если будет доступ в центр; иначе пн ~7:00 до
--     прихода детей. Запасной вариант — Sonia по этим трём строкам.
--     Токен НЕ пересылать в мессенджере; вводить на планшете с экрана консоли.
-- ---------------------------------------------------------------------------
