...
-- ⚠️ ЗАВИСИМОСТЬ: применять ПОСЛЕ 20260719c_safepass_teacher_nullable.sql.
-- teacher_id/teacher_name объявлены NOT NULL без default, а заявка родителя по
-- определению создаётся до появления учителя. Этот файл — SECURITY DEFINER, он
-- снимает стену прав (42501), но упрётся во вторую, независимую: NOT NULL.
-- Найдено Николаем на read-back'е 20260719 до применения чего-либо.
