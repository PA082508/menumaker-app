...
## 0.6. REDLINE 2026-07-26 — второй шаблон, правило AM/PM, старшинство образца

**Слово Николая 26.07. Эти строки имеют приоритет над §1–§2 там, где расходятся.**

**(1) Старшинство.** **Официальный образец побеждает спеку.** Спека описывает механику;
форма листа берётся с присланного образца байт-в-байт. Любая правка спеки под образец —
redline отдельной строкой по слову, а не молчаливое переписывание §1.

**(2) Шаблонов ДВА, не один.**
- **(а) regular** — `# · Child's Name · DOB · Mon–Fri × in|out · Schedule Hours`, учитель в
  шапке. Совпадает с §1 — расхождение снято, §1 в силе.
- **(б) SCHOOL-AGE** — каждый день делится на **am/pm × in/out** (четыре графы в дне), в
  шапке **до трёх учителей**. Это ВТОРОЙ печатный шаблон, а не режим первого.

**(3) Правило AM/PM — графа = ФАКТИЧЕСКАЯ половина дня события.**
Событие до полудня → графа **AM**, после полудня → графа **PM**. Граница — **12:00
ЛОКАЛЬНОГО времени центра**, тем же рельсом, что и сутки: `menumaker.center_local_day_start()`
(канон «сегодня» = локальный день центра, 26.07).

**Спец-режимов НЕТ** — маппинг сам даёт правильный лист в каждом случае:
- учебный день = 4 события → 4 графы (AM-in, AM-out, PM-in, PM-out);
- каникулы / полный день в саду = AM-in + PM-out, две графы, остальные пусты;
- ребёнок пришёл только после обеда = **обе** отметки попадают в PM.

Печатный шаблон school-age следует этому маппингу и ничего не додумывает: пустая графа
означает «события не было», а не «данные потеряны».

---

## 0.7. ОБРАЗЦЫ — приобщены 2026-07-26, прочитаны с диска

Файлы в репозитории (первоисточник печати; **образец побеждает спеку**, §0.6):

- [`docs/samples/attendance/Attendance_Ridge_-_Red.pdf`](../samples/attendance/Attendance_Ridge_-_Red.pdf) — **(а) regular**, недельная сетка
- [`docs/samples/attendance/Attendance_Ridge_-_SA2.pdf`](../samples/attendance/Attendance_Ridge_-_SA2.pdf) — **(б) school-age**, am/pm × in/out, три учителя в шапке
- [`docs/samples/attendance/Attendance_Ridge_-_Bus-Wickliffe1.pdf`](../samples/attendance/Attendance_Ridge_-_Bus-Wickliffe1.pdf) — **(в) водительский** Routine Trip Attendance Checklist

**ВЕРДИКТ (финальный, по прочтении): все три — HTML-реплика 1:1, оверлей не нужен.** Ни на одном
нет герба, номера формы, штрих-кода или печатной подложки с фиксированной геометрией: это
табличные бланки центра, сгенерированные из таблицы. Оверлей платил бы калибровкой (у DCY 01234 —
четыре круга) и ломался бы при любом изменении числа строк; реплика печатается 1:1 и тянется под
любой размер группы.

### Что несёт каждый образец (замерено по листам)

**(а) Red.** Шапка: `Weekly Attendance Report` · месяц · год · **`Red Room`** · `Teacher(s):` (одно
имя). Сетка: `#` · `Child's Name` · **DOB** · Mon–Fri, у каждого дня **две отдельные колонки
`in` и `out`** · `Schedule Hours`. Заполнены имя, DOB и часы; `in`/`out` пустые. Под списком —
**пустые строки до конца листа** (10 детей, ~24 строки): реплика сохраняет добивку пустыми
строками.

**(б) SA2.** Шапка: год и месяц **в обратном порядке** против (а) (`2026` над `July`), комната,
`Teacher(s):` — **три имени**. Сетка: `#` · `Child's Name` · DOB · Mon–Fri, у каждого дня
**`am` и `pm`**, а под ними подзаголовок **`in/out`**, и в каждой ячейке стоит **`/`** — то есть
время прихода и ухода пишутся **в одной ячейке через слэш**, а не в двух колонках. 29 детей.

**(в) Bus-Wickliffe.** Шапка: `Play Academy` · `Routine Trip Attendance Checklist` · **`Wickliffe`** ·
месяц · год. Сетка: `#` · левая колонка с двумя служебными строками — **`Departure / Arrival Time`**
и **`Driver's name`** — далее строки детей. По дням Mon–Fri, у каждого дня **`am` / `pm`**, метка
строки **`OnBus/Off`**, в ячейках **`/`**. **Колонки SCHOOL на бланке НЕТ.**

### Правки реплик, ратифицированные отдельно

- **`Rainboow` → `Rainbow`** в (б): опечатка исходника, в реплике пишем правильно.
- **Пустые `Schedule Hours`** (в (б) — строка Phillippone) реплика показывает **пустыми**, ничего
  не изобретая.

### ⚠️ Два вопроса, которые я не решаю сам

1. **Опечатки в шапках дней:** на обоих классных листах и на автобусном стоит **`Wen`** вместо
   `Wed` и **`The`** вместо `Thu`. Правило «образец побеждает спеку» велит копировать; ратификация
   `Rainboow → Rainbow` показывает, что опечатки исходника мы всё же правим. **Нужно слово: правим
   `Wen`/`The` или копируем как есть.**
2. **Автобусный лист — НЕДЕЛЬНЫЙ, а не «один на рейс».** Он повторяет ту же ось Mon–Fri × am/pm,
   что и классные, и держит **одну** пару am/pm на день. Значит печатная единица — **лист на
   АВТОБУС за неделю**, где рейс = одна колонка, а два параллельных рейса = два листа (по одному на
   автобус). Ранее записанное «лист печати — один на рейс» с образцом расходится; по канону §0.6
   выигрывает образец. **Остаётся один нерешённый случай: один водитель, один автобус, две ходки в
   одну половину дня — им обеим достаётся одна колонка.**
