...
### Два вопроса — ЗАКРЫТЫ словом Николая 27.07

1. **Опечатки в шапках дней — ПРАВИМ.** В репликах пишем **`Wed`** и **`Thu`**, хотя на всех трёх
   образцах стоит `Wen` и `The`. Тем же основанием, что `Rainboow → Rainbow`: опечатку исходника
   реплика не тиражирует. Правило «образец побеждает спеку» касается **структуры и состава**
   листа, не орфографии.
2. **Двойная ходка в одну половину дня — СЛЭШЕМ в одной колонке.** Один водитель, один автобус,
   две ходки в один am или pm пишутся **в ту же колонку через слэш** — ровно тем приёмом, каким
   образцы уже держат пару значений в одной ячейке (`in/out`, `OnBus/Off`). Второй колонки и
   второго листа не заводим.
