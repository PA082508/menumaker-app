...
create table menumaker.attendance_records (
  ...
  -- Через session_id восстанавливается кто принял / когда / каким устройством.
  in_session_id  uuid references menumaker.safepass_sessions(id),
  out_session_id uuid references menumaker.safepass_sessions(id),
  -- PIN-подпись. Обязательна и для source='manual' тоже.
  marked_by_staff uuid,                    -- staff.id, разрешённый PIN-ом → ЧЕЛОВЕК
...
handoff-событии, и дублировать их значит завести второй источник той же правды.
Для `source='manual'` `*_session_id` = NULL, и `marked_by_staff` — единственная подпись.

`marked_by_staff` пока **nullable**: см. §5 вопрос 3 (человек без `staff`-строки).
Если решение — «без PIN не отмечаем», колонка станет `not null` **до первой живой
записи**, без обратной миграции.
...
Решённое ушло в шапку. Осталось три — и первые два нужны до кода Волны 2:

1. **Несколько входов/выходов за день.** `unique (child_id, on_date)` и образец владельца дают одну пару `in|out`. Но `safepass_sessions` допускает несколько `drop_off`/`pick_up` (и `transfer`). Ребёнка забрали и привели снова — печатаем первый in и последний out, или несколько строк?
2. **Правки — append-only лог или UPDATE поверх?** Посещаемость регулируемая; правка учителем поверх SafePass-времени должна оставлять след (кто, когда, что было). Прецедент есть — `determination_log` ([enrollmentApprove.ts:257](../../src/lib/enrollmentApprove.ts#L257)). Рекомендую append-only.
3. **Учитель без `staff`-строки** (новый человек) — отметка невозможна вовсе, или пишется без PIN с явной пометкой «без подписи»? (дублируется в identity-спеке §6.4 — ответ нужен один).
4. **SafePass пуст** (§3.2.2): 0 устройств, 0 PIN-ов, 0 сессий — при том, что пилот Wickliffe по родительскому письму объявлен обязательным с 15.07. Пилот не стартовал, или идёт мимо системы? От ответа зависит, есть ли у производной модели вход в Волне 3, или Attendance какое-то время живёт на одной ручной отметке.
5. **Живой прямой `.update()` в `SafePassTeacherPage`** (§3.2.1) — закрываем в пользу RPC до Attendance, или Attendance стартует, зная, что часть `teacher_id` — сервис-аккаунты? Рекомендую закрыть до: смешанные id в одной колонке чистятся только руками.
