...
// Asserts Submit is PRESENT, ENABLED (with ?center=), and VISIBLE (real contrast),
// and that the page raised ZERO JS exceptions. Presence alone is NOT enough:
// a white-on-white Submit passes a presence check and fails a human (2026-07-14).
...
const FORMS = [
  ['enroll v9', '/forms/1-data-sources/CACFP_Enrollment_v9.html'],
  ['iea v6', '/forms/1-data-sources/IEA_FY2026-27_v6.html'],
...
console.log(pad('FORM', 19), pad('present', 8), pad('enabled', 8), pad('bg', 18), pad('contrast', 9), pad('errs', 5), 'VERDICT');
console.log('-'.repeat(96));
for (const [name, p] of FORMS) {
  ...
  const r = await pg.evaluate(() => {
    ...
    return { present: true, enabled: !s.disabled, bg: cs.backgroundColor, fg: cs.color, w: rc.width, h: rc.height };
  });
  let verdict = '✅ PASS';
  if (!r.present) { verdict = '❌ FAIL: absent'; fails++; }
    ...
    console.log(pad(name, 19), pad('yes', 8), pad(r.enabled, 8), pad(r.bg, 18), pad(d.toFixed(1), 9), pad(errs.length, 5), verdict);
    await ctx.close(); continue;
  }
  console.log(pad(name, 19), pad('NO', 8), pad('-', 8), pad('-', 18), pad('-', 9), pad(errs.length, 5), verdict);
  await ctx.close();
}
await b.close(); if (srv) srv.close();
console.log('\n' + (fails ? '❌ ' + fails + ' FORM(S) FAILED' : '✅ ALL ' + FORMS.length + ' FORMS PASS — Submit present, enabled, visible, 0 exceptions'));
