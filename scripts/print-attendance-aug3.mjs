// print-attendance-aug3.mjs — печатные бланки Attendance по классам, неделя 03–07.08.2026.
//
...
const WEEK_LABEL = 'Aug 3 – Aug 7, 2026'
...
function sheet(center, room, kids, no, of) {
  const head = DAYS.map((d, i) => {
    ...
    return `<th colspan="2" class="day">${d}<div class="dnum">${Number(m)}/${Number(dd)}</div></th>`
  }).join('')
