...
//
// ⭐ ГЛАВНОЕ ПРАВИЛО (31.07, выше всех прочих о формах): СТРОИШЬ ФОРМУ — БЕРИ ТО,
// ЧТО НА ЭКРАНЕ. Экран уже соблюдает все четыре правила ниже; лист, собранный мимо
// него, нарушил их все. Поэтому вход этого скрипта обязан приходить ТОЙ ЖЕ дорогой,
// что и экран: имя — displayChildName, порядок — возрастные полосы экрана, итоги —
// правило возмещения экрана. Приёмка: положить рядом экран и лист; расхождение в
// порядке имён, порядке строк или составе итогов = лист не принят.
// Долг: скрипт повторяет логику экрана на JS вместо общего кода — пока лист живёт
// отдельным генератором, это место и есть риск расхождения; закрывается переносом
// сборки листа в приложение (спека §15).
//
