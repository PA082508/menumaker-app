...
// ─── Срок: вычисленный против введённого ─────────────────────────────────────
/**
 * Расхождение между сроком, который вывело правило, и сроком, введённым руками.
 *
 * ЗАЧЕМ ЗАПИСЫВАТЬ. Правило CACFP даёт 12 месяцев от даты документа до конца
 * месяца, и в большинстве случаев оно право. Но на бланке бывает напечатан СВОЙ
 * срок, и тогда прав бланк — спорить с бумагой нельзя. Разница между этими двумя
 * датами это не ошибка ввода, а ФАКТ о документе, и через год, когда ребёнок
 * «внезапно» просрочен на два месяца раньше ожидаемого, единственный способ
 * понять почему — найти в журнале, что срок был взят с бланка, а не вычислен.
 *
 * Возвращает строку для журнала или null, когда расхождения нет (введённое
 * совпало с вычисленным — записывать нечего).
 */
export function expiryOverrideNote(computed: string | null, entered: string | null): string | null {
  if (!entered || !computed) return null
  if (entered === computed) return null
  return `Expiry taken from the form: ${entered} (the 12-month rule from the document date gives ${computed}).`
}
