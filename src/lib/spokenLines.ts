 ...
 * (экран счёта говорит по-русски). Длинную полосу вслух не читают: голос несёт
 * ровно то, что человек должен сделать, а подробности остаются на экране.
 */
export function spokenMarkRefusal(childName: string): string {
  const child = childName.trim()
  return child ? `Отметка не сохранена: ${child}. Отметьте заново.` : 'Отметка не сохранена. Отметьте заново.'
}
