// ============================================================
// InstructionsPage.tsx — route /instructions
// Document Hub — instructions, guides, policies
// with QR codes and online BYOD signing
// ============================================================
...
const DOCS: DocItem[] = [
  ...
  { id: 'safepass-teacher', title: 'SafePass — Teacher Quick Guide (Wickliffe)', description: 'iPad guide: drop-off, pick-up, unknown person protocol, quick reference table.', audience: 'teacher', category: 'SafePass', date: 'July 2026', comingSoon: true },
  { id: 'safepass-concept', title: 'SafePass — Complete Concept Document v1.1', description: 'Philosophy, legal basis, chain of custody, transportation, school partnership, closed loop.', audience: 'director', category: 'SafePass', date: 'July 2026', comingSoon: true },
  { id: 'byod-policy', title: 'BYOD Policy — HR-BYOD-001', description: 'Company policy: voluntary participation, $20/month stipend, privacy protections, 9 sections.', audience: 'staff', category: 'BYOD', date: 'July 2026', comingSoon: true },
  ...
  { id: 'timelog', title: 'Daily Time Log — Teacher Guide', description: 'CACFP labor time log — how to complete your monthly record.', audience: 'teacher', category: 'Staff', comingSoon: true },
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:1000,overflowY:'auto',display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'20px 12px'}}>
      <div style={{background:'#fff',borderRadius:16,width:'100%',maxWidth:540,boxShadow:'0 24px 80px rgba(0,0,0,0.25)'}}>
        ...
        <div style={{display:'flex',borderBottom:'1px solid #f0f0f0'}}>
          {[1,2,3,4].map(n=>(
            <div key={n} style={{flex:1,textAlign:'center',padding:'10px 4px',fontSize:11,fontWeight:600,
              color:n===step?'#1a5c3f':n<step?'#059669':'#9ca3af',
              borderBottom:`3px solid ${n===step?'#1a5c3f':n<step?'#059669':'transparent'}`}}>
              <span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:20,height:20,borderRadius:'50%',marginRight:4,fontSize:10,background:n===step?'#1a5c3f':n<step?'#059669':'#e5e7eb',color:n<=step?'#fff':'#9ca3af'}}>
                {n<step?'✓':n}
              </span>{STEPS[n]}
            </div>
          ))}
        ...
        <div style={{padding:24}}>
          {step===1&&<div>
            <p style={{fontSize:13,color:'#6b7280',marginBottom:16}}>Please fill in your information to begin.</p>
          ...
          {step===2&&<div>
            <div style={{height:300,overflowY:'auto',border:'1px solid #d1fae5',borderRadius:10,padding:'14px 16px',fontSize:13,lineHeight:1.7,color:'#374151',background:'#f8faf8',marginBottom:14}}>
              <p><strong>This Agreement</strong> is between <strong>Play Academy Inc.</strong> ("Company") and the Employee named above.</p>
              <p style={{marginTop:10}}><strong>Art. 1. Purpose.</strong> The Employee voluntarily agrees to use their personal mobile device for authorized work-related functions including SafePass. <strong>The Play Academy app works ONLY on registered and authorized devices.</strong></p>
              <p style={{marginTop:10}}><strong>Art. 2. Compensation.</strong> Monthly stipend of <strong>$20.00</strong> included in regular paycheck. Ceases upon termination of Agreement.</p>
              <p style={{marginTop:10}}><strong>Art. 3. Employee Obligations.</strong> Maintain device; keep charged; enable screen lock; install only authorized apps; not share credentials; report loss/theft immediately; allow app removal upon termination.</p>
              <p style={{marginTop:10}}><strong>Art. 4. Company Limitations.</strong> Play Academy will NOT access personal communications, photos, or non-work apps. Work data stored on Company servers only. Apps removed within 24 hours of termination.</p>
              <p style={{marginTop:10}}><strong>Art. 5. Confidentiality.</strong> All child-related data is confidential under Ohio and federal privacy laws. No disclosure to unauthorized persons.</p>
              <p style={{marginTop:10}}><strong>Art. 6. Termination.</strong> Either party may terminate: Employee with written notice (Company device within 5 days); Company immediately upon violation; automatically upon employment termination.</p>
              <p style={{marginTop:10}}><strong>Art. 7. Governing Law.</strong> Ohio law. Disputes in Cuyahoga County courts.</p>
            </div>
            <div style={{display:'flex',alignItems:'flex-start',gap:10,padding:12,background:'#f0f7f4',borderRadius:8,marginBottom:12}}>
              ...
              <label htmlFor="ag" style={{fontSize:13,color:'#374151',cursor:'pointer'}}>I have read and fully understand this Agreement. I voluntarily agree to its terms.</label>
            </div>
          ...
          {step===3&&<div>
            <p style={{fontSize:13,color:'#6b7280',marginBottom:14}}>Sign below using your finger (mobile) or mouse (desktop).</p>
            ...
            <div style={{marginTop:14,padding:12,background:'#f8faf8',borderRadius:8,fontSize:12,color:'#6b7280',lineHeight:1.6}}>
              ...
              This electronic signature has the same legal effect as a handwritten signature under Ohio law.
            </div>
          ...
          {step===4&&<div style={{textAlign:'center',padding:'16px 0'}}>
            ...
            <p style={{color:'#6b7280',fontSize:13,lineHeight:1.6}}>Your BYOD Agreement has been submitted and saved securely. Your director will countersign shortly.</p>
            <div style={{background:'#f0f7f4',borderRadius:10,padding:16,margin:'16px 0',textAlign:'left',fontSize:13}}>
              {[['Employee',f.name],['Center',f.center],['Device',f.device],['Phone',f.phone],['Signed',new Date().toLocaleString('en-US')],['Reference ID',refId]].map(([l,v])=>(
            ...
            <div style={{background:'#fef3c7',borderRadius:10,padding:14,fontSize:13,color:'#92400e',textAlign:'left',marginBottom:16}}>
              ⏳ Your BYOD stipend of <strong>$20.00/month</strong> begins with your next paycheck after director confirmation.
            </div>
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:16,padding:32,maxWidth:300,width:'100%',textAlign:'center',boxShadow:'0 24px 80px rgba(0,0,0,0.25)'}}>
        ...
        <div style={{marginTop:6,fontSize:10,color:'#9ca3af',wordBreak:'break-all'}}>{url}</div>
      ...
      <div style={{background:'linear-gradient(135deg,#1a5c3f,#2d7a56)',borderRadius:12,padding:'16px 20px',marginBottom:20,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        <div>
          ...
          <div style={{color:'rgba(255,255,255,0.8)',fontSize:13,marginTop:2}}>
            {count===null ? 'Loading...' : `${count} employee${count!==1?'s':''} signed`} · Director countersignature pending
          </div>
        </div>
        <button onClick={()=>setSignOpen(true)} style={{padding:'10px 20px',background:'#fff',color:'#1a5c3f',border:'none',borderRadius:8,fontWeight:700,fontSize:14,cursor:'pointer',fontFamily:'inherit'}}>
          Sign Now →
        </button>
      </div>
