-- Background duplicate-child detector — FUZZY arm (block 8b of refresh_action_items).
-- Follow-up to 20260707c (which shipped the EXACT arm and pre-installed
-- fuzzystrmatch/unaccent for exactly this). Approved 2026-07-08 (Nikolay).
--
-- SCOPE — deliberately narrow: ACTIVE ↔ ACTIVE only. Near-identical names
-- (bounded Levenshtein ≤ 2 on the normalized name) with equal-or-one-null DOB,
-- surfaced as a data_quality action item so a director can merge the typo pair.
-- active↔inactive is intentionally EXCLUDED: a returning (inactive) child is
-- caught by the ADD CHILD 2.0 router at intake, and inactive fiscal stubs are
-- owned by the Skeleton Reconciliation worksheet (its own fuzzy keeper arm).
-- Keeping the detector active-only holds that boundary and avoids double-owning.
--
-- Detector only READS roster + WRITES action_items — never mutates roster/frp.
-- This CREATE OR REPLACE reproduces 20260707c's function verbatim and adds
-- block 8b + the v_dupf counter + a 'duplicates_fuzzy' return field.
...
declare
  ...
  v_sub int := 0; v_appr int := 0; v_licnd int := 0; v_dup int := 0; v_dupf int := 0;
...
begin
  ...
  -- ===== 8) DUPLICATE ROSTER CHILDREN (exact name) =====
  update menumaker.action_items set status='resolved', resolved_at=now(), updated_at=now()
  ...
  -- ===== 8b) FUZZY DUPLICATE (active ↔ active ONLY; typo / spelling variants) =====
  -- Same source/clear as block 8 above (cleared once at block 8 start). Bounded
  -- Levenshtein ≤ 2 on the normalized name, min length 6 to avoid short-name
  -- noise, exact pairs excluded (owned by block 8), DOB equal or one null.
  for rec in
    with active as (
      select r.id, r.center_id, r.child_name, r.birthday,
             menumaker.norm_name(r.child_name) as nn
    ...
    pairs as (
      select a.center_id, a.id id_lo, b.id id_hi,
             a.child_name name_a, b.child_name name_b,
             a.birthday dob_a, b.birthday dob_b,
             extensions.levenshtein(a.nn, b.nn) as dist
      from active a
      join active b on a.center_id=b.center_id and a.id < b.id
      where coalesce(a.nn,'') <> '' and coalesce(b.nn,'') <> ''
        and a.nn <> b.nn                                  -- exact pairs → block 8
        and length(a.nn) >= 6 and length(b.nn) >= 6       -- short-name noise guard
        and extensions.levenshtein(a.nn, b.nn) <= 2       -- 1–2 char typo
        and (a.birthday is null or b.birthday is null or a.birthday = b.birthday)
    )
    select p.*, ct.name center_name
    from pairs p left join menumaker.centers ct on ct.id = p.center_id
  loop
    perform menumaker.raise_action_item(
      p_org_id, 'data_quality', 'high',
      'Possible duplicate child (similar names): '||rec.name_a||' / '||rec.name_b||' ('||coalesce(rec.center_name,'?')||')',
      'Two active roster records have near-identical names (edit distance '||rec.dist||')'||
        case when rec.dob_a is null or rec.dob_b is null
               then ' and a missing birthday — likely a typo or skeleton variant.'
             else ' and the same birthday '||to_char(coalesce(rec.dob_a,rec.dob_b),'MM/DD/YYYY')||'.' end||
        ' Reconcile: keep one record, merge, and repoint eligibility. Do not delete without review.',
      'duplicate_scan', 'roster', rec.id_hi,
      'dupf:'||rec.center_id||':'||rec.id_lo||':'||rec.id_hi, null);
    v_dupf := v_dupf + 1;
  end loop;
  ...
  return jsonb_build_object('licenses',v_lic,'claims',v_clm,'documents',v_doc,'eligibility',v_iea,
                            ...
                            'duplicates',v_dup,'duplicates_fuzzy',v_dupf);
end $function$;
