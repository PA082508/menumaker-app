...
--
-- ✅ ПРИМЕНЕНО НА ПРОДЕ 2026-08-03 21:33 UTC по слову владельца, после того как
--    светофор по центрам дал ДА (Highland 🟢, Pearl 🟢, Ridge 🟢). Рубеж «до»:
--    1388 июльских строк, хеш 731401280bff07cb80aa1e03ef3b83d2 — после миграции
--    тот же. Ниже сохранён разбор трёх отказов предпроверки 02.08 и то, как
--    каждый из них был снят; это история решения, а не список открытых дел.
--
--    ⚠️ ВТОРОЙ РАЗ НЕ ПРИМЕНЯТЬ. Файл идемпотентен по форме (if not exists /
--    if exists / create or replace), но следующая правка ключа — новый файл.
--
--      20260731b (схлопывание сирот) применён 01.08 — расщеплённых групп 0,
--      гард ниже прошёл.
--
-- 🔴 ПРЕДПРОВЕРКА 02.08 ДАВАЛА СТОП. Тогда ключ НЕ ПОШЁЛ — три отказа:
--
--   (1) ЖИВОЙ ЭКРАН ДИРЕКТОРА ПИШЕТ В СНИМАЕМЫЙ ИНДЕКС.
--       MealCountDirectorPage.tsx:243–249 (сегодня в main, на проде) в ветке
--       «строки ещё нет» делает
--           .upsert(..., { onConflict: "classroom_id,child_name,monday_date" })
--       — ровно тот индекс, который снимает п. 3 этого файла. Частичный индекс
--       Postgres выводит только при повторении предиката в запросе; у этого
--       вызова предиката нет. Результат — SQLSTATE 42P10, и он ПРОГЛАТЫВАЕТСЯ
--       пустым catch на строке 252: галочка молча откатывается.
--       Тот же вызов вдобавок не шлёт roster_id вовсе.
--       ⇒ Клиент обязан быть переведён и ВЫКАЧЕН ДО этого файла, а не после.
--
--   (2) ПЛАНШЕТЫ НЕ ОТЧИТАЛИСЬ ВЕРСИЕЙ. Условие владельца от 01.08 — «каждое
--       отмечающее устройство отчиталось свежим app_version». На 02.08: 8 из 8
--       рабочих планшетов в состоянии ⏳ (после выкатки 31.07 22:09 UTC ни один
--       не отмечал). Единственный, кто отчитался, — браузер проверок, и он из
--       гейта исключён. Сборки старее f25d55d (10.07) гибнут тем же 42P10.
--       Замер частоты: строки недели рождаются по первому тапу на ребёнка, а
--       неделя 03.08 сегодня пуста ⇒ в понедельник у Ridge это ~140 отказов
--       подряд, невидимых, в месяце заявки.
--
--   (3) ЭТОТ ФАЙЛ ТИХО ОТКЛЮЧАЛ ДЕТЕКТОР ВЕРСИЙ — исправлено 02.08, см. п. 4(c).
--
--   ⇒ ПРАВИЛЬНЫЙ ПОРЯДОК: перевести клиента → выкатить → дождаться, что все 8
--     планшетов отчитались свежей версией (вахта понедельника) → и только тогда
--     этот файл. Снимок июля на момент «до» уже взят: ярлык pre-20260731a-july,
--     1388 строк, хеш 4682ba46fb6f9a0d95271a5b1ae95947.
--
--   КАК ЭТО ЗАКОНЧИЛОСЬ (вахта понедельника 03.08):
--     (1) снят выкаткой 02.08 — прямая запись с директорского экрана убрана,
--         оба экрана пишут через sync_meal_marks;
--     (2) снят вахтой. Счёт устройств оказался неверен по форме: «8 планшетов»
--         включало старые id, которые уже не существуют. Планшет, у которого
--         почистили хранилище, возвращается НОВЫМ id, а прежний навсегда остаётся
--         в журнале без версии и держит гейт вечно. Правило владельца 03.08:
--         считать только устройства, отмечавшие СЕГОДНЯ; старый id, за день не
--         проявившийся, — перерождённый, вычёркивается. По этому счёту гейт
--         открылся: Highland 2/2, Pearl 2/2, Ridge 1/1 на e76a158;
--     (3) исправлено 02.08 — app_version в журнале, см. п. 4(c).
--
...
begin
  ...
  loop
    ...
    -- (c) Журнал точки обслуживания — идемпотентен по uuid очереди.
    --     ⚠️ ПРАВКА 02.08: добавлена app_version. Этот файл написан 31.07 ДО того,
    --     как 20260731g (применён 31.07 в 22:03 UTC) завёл колонку версии клиента.
    --     Применить файл в исходном виде значило бы ТИХО ОТКЛЮЧИТЬ ДЕТЕКТОР ВЕРСИЙ —
    --     то есть уничтожить единственный инструмент, который говорит, когда
    --     мигрировать безопасно. Ровно тот прибор, которым этот файл и гейтится.
