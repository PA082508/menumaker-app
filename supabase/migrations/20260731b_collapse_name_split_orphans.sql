...
--
-- ✅ ПРИМЕНЕНО 2026-08-01, 15:37–16:11 UTC. Это ШАГ (2) плана, он идёт ПЕРЕД 20260731a.
--
--    ЧТО ПОЛУЧИЛОСЬ (читка из прода 02.08): в menumaker.meal_week_records_merged
--    24 строки, 23 из них несут подпись директора; расщеплённых групп в живой
--    сетке — 0. Ровно то, что файл обещал.
--
--    ⚠️ ЗАПИСИ В ЖУРНАЛЕ МИГРАЦИЙ НЕТ. В supabase_migrations.schema_migrations
--    этот файл не значится — применён мимо журнала. Поэтому до 02.08 репозиторий
--    считал его заготовкой, а заголовок утверждал «НЕ ПРИМЕНЕНО»: заголовок файла
--    не является свидетельством о базе, свидетельство — сама база.
--
--    ⚠️ НЕОБРАТИМО ДЛЯ ВЫЖИВШИХ СТРОК. Слияние писало greatest(своя клетка,
--    чужая клетка); архив хранит дословно только ПРОИГРАВШИЕ строки, доклеточные
--    значения выживших не сохранены нигде. Состояние июля «до 20260731b» задним
--    числом не восстановить.
--
-- ⚠️⚠️ ГАРД СВЕЖЕСТИ (сработал бы, если бы директор правил руками). 31.07 с 12:19
--      до 14:12 UTC Татьяна тронула 174 строки недели в 18 классах и подписала
--      неделю 13.07 — ровно те строки, которые файл собирался слить. Гард ниже
--      роняет файл сам, если хоть одна целевая строка правилась
